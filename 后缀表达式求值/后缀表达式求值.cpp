...
int evaluatePostfix(char *postfix) 
{
	...
	{
        char c = postfix[i];
        if (ifOperand(c))  //如果当前字符是操作数，则将连续的数字字符转换为对应的整数值，并将该操作数入栈
		{
            int operand = 0;
            while (c != '#') 
			{
                operand = operand * 10 + (c - '0'); //(c - '0')将字符型的数字转换为对应的整数值
                c = postfix[++i];
            }
            push(&stack, operand);
        } 
		else  //如果当前字符是操作符，则从栈中弹出两个操作数，并根据当前操作符进行相应的操作
		{
