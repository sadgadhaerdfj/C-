...
int main() 
{
    ...
    for (int c = 0; c < num * 2 - 1; c++) 
	{
		...
		{//如果当前节点的下标大于等于输入的节点个数 num，说明这是构建哈夫曼树时新增的节点（非输入节点）
			...
			else //如果当前节点有父节点（不是根节点），则输出节点的权重、父节点、左子节点和右子节点的信息。
			{
                printf("第%-2d个节点  权重为%-5d  父节点为第%-2d个  左子节点为第%-2d个  右子节点为第%-2d个\n",
                       c + 1, ps[c].weight, ps[c].parent + 1, ps[c].lchild + 1, ps[c].rchild + 1);
            }
        } 
		else //如果当前节点的下标小于输入的节点个数 num，说明这是原始输入的节点，它们没有左子节点和右子节点，只输出节点的权重和父节点的信息
		{
            printf("第%-2d个节点  权重为%-5d  父节点为第%-2d个 (无左右子节点)\n", c + 1, ps[c].weight, ps[c].parent + 1);
        }
    }	
    return 0;
}
