...
//思路：用名字、学号、ID排序，分别存入三个文本文件里，再用二分法 
...
void swap(Student *xp, Student *yp)//交换两个元素的值 ，用于排序 
{
    Student temp = *xp;
    *xp = *yp;
    *yp = temp;
...
void ID_digit_Sort(Student students[], int n)//学号冒泡排序 （升序） 
{
	int i, j;
	for (i = 0; i < n-1; i++)
	{
		for (j = 0; j < n-i-1; j++)
		{
			...
			{
				swap(&students[j], &students[j+1]);//大的放前面 
			}
		} 
	}
}

void digit_Sort(Student students[], int n)//平均分冒泡排序 （升序） 
{
	int i, j;
	for (i = 0; i < n-1; i++)
	{
...
void characters_Sort(Student students[], int n)//姓名冒泡排序 
{
    setlocale(LC_COLLATE, "zh_CN");    //使用setlocale函数将本地化环境设置为中文（"zh_CN"），以确保正确比较中文字符的顺序
    for (int i = 0; i < n-1; i++)
    {
        for (int j = 0; j < n-i-1; j++)
        {
            if (strcoll(students[j].name, students[j + 1].name) > 0)//strcoll函数会根据当前的区域设置（locale）对两个字符串进行字典排序
			                                                        //（students[j].name在students[j + 1].name之前返回负值；反之，返回正值；相同返回0） 
            {
...
void SearchTotal(Student students[], const char *fileName, int *count)//第一次弄的是总成绩单的，储存学号/姓名/平均值 
{
	// 将之前的文件指针代码移到这里
    ...
    char line[100];
    //使用fgets函数逐行读取文件内容，每次读取一行数据并存储在line数组中。如果成功读取到一行数据，则进入循环体
