...
{
    node_1 gdx[10000];
    FILE* fp = fopen("三国演义权重.txt", "r"); // 打开名为“三国演义权重.txt”的文件进行读取
    if(fp == NULL)
	{
        printf("打开失败"); 
        return 0;	
	...
	{
        char *token = strtok(ch, ","); // 使用逗号分隔每行内容
        int count = 0;
        // 遍历分隔后的字段
        while (token != NULL) 
		{
			...
			{
                case 0:
                    strcpy(ps[count1].name, token); // 将第一个字段作为节点名称存储
                    break;
                case 1:
                    ps[count1].weight = atoi(token); // 将第二个字段转换为整数并存储为节点权重
                    break;
                default:
                    break;
            }
...
{
    result_min = find_min(c); // 找到权重最小的两个节点
...
FILE* fp1 = fopen("三国演义哈夫曼编码.txt", "w"); // 打开文件 "三国演义哈夫曼编码.txt" 进行写入
...
for (int c = 0; c < num * 2 - 1; c++) 
{
    if (c >= num) {
        ...
        fprintf(fp1, "%s\t出现次数为%-5d\t哈夫曼编码为%s\n", ps[c].name , ps[c].weight, ps[c].code);
        // 将节点的名称、权重和哈夫曼编码写入文件
    }
}
